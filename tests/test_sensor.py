...
async def test_async_setup_entry_no_children(hass: HomeAssistant):
    ...
    from custom_components.firefly_cloud.const import DOMAIN, CONF_SCHOOL_NAME, CONF_HOST, CONF_DEVICE_ID, CONF_SECRET, CONF_USER_GUID, CONF_CHILDREN_GUIDS
...
async def test_async_setup_entry_multiple_children(hass: HomeAssistant):
    ...
    from homeassistant.config_entries import ConfigEntry
    from custom_components.firefly_cloud.const import DOMAIN, CONF_SCHOOL_NAME, CONF_HOST, CONF_DEVICE_ID, CONF_SECRET, CONF_USER_GUID, CONF_CHILDREN_GUIDS, CONF_CHILDREN_GUIDS
    ...
    coordinator.data = {
        ...
        "children_data": {
            "child-1": {"events": {"today": [], "week": []}, "tasks": {"upcoming": [], "due_today": [], "all": [], "overdue": []}},
            "child-2": {"events": {"today": [], "week": []}, "tasks": {"upcoming": [], "due_today": [], "all": [], "overdue": []}},
            "child-3": {"events": {"today": [], "week": []}, "tasks": {"upcoming": [], "due_today": [], "all": [], "overdue": []}},
        },
    }
...
async def test_sensor_device_info_consistency(mock_coordinator, mock_config_entry):
    ...
    for sensor in sensors[1:]:
        ...
        assert sensor.device_info["identifiers"] == base_device_info["identifiers"]
        assert sensor.device_info["name"] == base_device_info["name"]
